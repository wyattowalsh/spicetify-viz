...
function SearchAIInput(props: FormHTMLAttributes<HTMLFormElement>) {
  ...
  const onStart = (e?: FormEvent<HTMLFormElement>) => {
    e?.preventDefault();
    handleSubmit(e);
  };
      ...
      <Input
        ...
        onKeyDown={(event: KeyboardEvent<HTMLTextAreaElement>) => {
          if (!event.shiftKey && event.key === 'Enter') {
            onStart(event as unknown as FormEvent<HTMLFormElement>);
    ...
    <div>
      ...
      {links && links.length > 0 ? (
        <div className="mt-2 flex flex-row flex-wrap items-center gap-1">
          {links.map(
            (
              item: z.infer<typeof ProvideLinksToolSchema>['links'][number],
              i: number,
            ) => (
              <Link
                key={i}
...
function Pre(props: ComponentProps<'pre'>) {
  ...
  let lang =
    codeProps.className
      ?.split(' ')
      .find((v: string) => v.startsWith('language-'))
...
function Content() {
  ...
  const messages = chat.messages.filter((msg: Message) => msg.role !== 'system');
